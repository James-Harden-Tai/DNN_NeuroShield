...
def model(x):
    ...
    return (cheat_net_cpu(torch.tensor(x).double()).numpy().argmax(1)).astype(np.int32)[
        0
    ]
...
def cheat_neuron_diff_cuda_2(a, b):
    ...
    a = out[:, 0, :]
    b = out[:, 1, :]
    return torch.where(a.flatten() != b.flatten())[0].cpu().numpy(), np.array(
        a.flatten().cpu().numpy(), dtype=np.uint8
    )
...
def find_decision_boundary(zero=None, one=None, tensor=False):
    ...
    last = 1e9
    while (
        torch.sum(torch.abs(zero - one)) > 1e-16
        and torch.sum(torch.abs(zero - one)) < last
    ):
...
def get_gradient_dir(x, cache={}, debug=False, step_size=1e-7, dimensions=None):
    ...
    for i in dimensions:
        ...
        for step in 10 ** np.arange(-7, 0, 0.33):
            xp2 = np.array(xp)
            xp2[i] += step
...
def get_gradient_dir_fast(x, cache={}, debug=False, step_size=1e-7, dimensions=None):
    ...
    ratios = (
        ((xp - boundary) / (step_size))[torch.arange(len(dimensions)), dimensions]
        .cpu()
        .numpy()
    )
