...
def analyze_df(df):
    ...
    df["votes-"] = (~df["Vote dOFF>dON"]).cumsum()
    df["alpha"] = df.apply(
        lambda x: get_alpha(x["votes+"], x["votes-"], x["nExp"]), axis=1
    )
    ...
    speed_cols_exist = (
        "speed_ON_mean1" in df.columns and "speed_OFF_mean1" in df.columns
    )
    speed_dir_cols_exist = (
        "speed_ON_dirT1" in df.columns and "speed_OFF_dirT1" in df.columns
    )
    ...
    _results = {
        ...
        "||vON||/||vOFF||": (
            np.mean(df["||vON||/||vOFF||"])
            if "||vON||/||vOFF||" in df.columns
            else np.nan
        ),
        "sON>sOFF": (
            100.0
            * (np.sum(df["speed_ON_mean1"].values > df["speed_OFF_mean1"]) / len(df))
            if speed_cols_exist
            else np.nan
        ),
        "sONdir>sOFFdir": (
            100.0
            * (np.sum(df["speed_ON_dirT1"].values > df["speed_OFF_dirT1"]) / len(df))
            if speed_dir_cols_exist
            else np.nan
        ),
...
if __name__ == "__main__":
    ...
    for lID in range(1, 5):
        ...
        df["sON>sOFF"] = df["sON>sOFF"].apply(
            lambda x: f"{x:.1f}%" if pd.notna(x) else "N/A"
        )
        df["sONdir>sOFFdir"] = df["sONdir>sOFFdir"].apply(
            lambda x: f"{x:.1f}%" if pd.notna(x) else "N/A"
        )
        ...
        # Replace NaN with a placeholder string before formatting
        df["||vON||/||vOFF||"] = df["||vON||/||vOFF||"].fillna("N/A")
