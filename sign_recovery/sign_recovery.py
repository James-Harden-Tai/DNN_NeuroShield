...
def main(argv):
    ...
    if args.layerID > len(weights):
        raise Exception(
            f"The provided target layer ID {args.layerID} exceeds the length of weights ({len(weights)}) in the model "
        )
    if args.layerID == 0:
        raise Exception(
            f"Only hidden layers can be attacked, not the input layer (layerID={args.layerID}). Please pass a layerID>0."
        )
    if args.layerID == len(weights):
        raise Exception(
            f"The output layer with layerID({args.layerID})=len(weights) cannot be attacked using this method."
        )
    if args.layerID == (len(weights) - 1):
        print(
            f"WARNING: The penultimate layer with layerID={args.layerID} should not be attacked with this method (since all toggles have to be in future layers)."
        )

    if not eval(args.nDebug):
        logger.debug(
            f"Check that model weights are dtype tf.float64: \t {model.weights[0].dtype}"
        )
    ...
    dual_point_id = 0
    if not ndebug:
        logger.debug(f"Load dual points from file: {args.filepath_load_x0}.")

    # 构造正确的文件路径：layerX_neuronY.npy
    # 旧代码错误地尝试加载一个目录，而不是文件
    dual_points_file = os.path.join(args.filepath_load_x0, f"layer{layerId}_neuron{neuronId}.npy")
    
    if not os.path.exists(dual_points_file):
        print(f"❌ 错误: 对偶点文件未找到!")
        print(f"   期望路径: {dual_points_file}")
        print(f"   请检查该文件是否存在，或运行find_duals.py来生成它。")
        raise FileNotFoundError(f"Dual points file not found: {dual_points_file}")

    X_DUAL = np.load(dual_points_file)
